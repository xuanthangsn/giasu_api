...
const getProvinces = async (req, res, next) => {
    try {
        const provinces = await db.sequelize.query("SELECT p.code, p.name , p.full_name , p.full_name_en ,au.full_name as administrative_unit_name FROM provinces p INNER JOIN administrative_units au ON p.administrative_unit_id = au.id ORDER BY code;", 
...
const getDistricts = async (req, res, next) => {
    ...
    try {
        const districts = await db.sequelize.query(`SELECT d.code, d.name , d.full_name , d.full_name_en ,au.full_name 
...
const getWards = async (req, res, next) => {
    ...
    try {
        const wards = await db.sequelize.query(`SELECT w.code, w.name , w.full_name , w.full_name_en ,au.full_name 
        as administrative_unit_name
        FROM wards w 
        INNER JOIN administrative_units au 
