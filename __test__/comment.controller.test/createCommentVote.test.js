...
describe("create comment vote", () => {
  describe("vote found,  equal vote type", () => {
    it("200 status code", async () => {
      jest.mocked(db.Vote.findOne).mockResolvedValue({ vote_type: 1 });
      jest.mocked(db.Vote.destroy).mockResolvedValue({});

      await supertest(app).post("/api/comment/1/1").expect(200);
    });
  });

  describe("vote found, not equal vote type", () => {
    it("200 status code", async () => {
      jest
        .mocked(db.Vote.findOne)
        .mockResolvedValue({ vote_type: "not equal" });
  ...
  describe("vote not found", () => {
    ...
    describe("query error", () => {
      it("500 status code", async () => {
        ...
        await supertest(app).post("/api/comment/1/1").expect(500);
      });
    });
});
